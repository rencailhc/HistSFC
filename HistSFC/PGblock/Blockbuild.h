...
HistNodeND *BlockLoad(string HistTab)
{
	...
	try
	{
		...
		while (rs->next())
		{
			...
			curnode->child = (HistNodeND*)0;
			curnode->neighbor = (HistNodeND*)0;
...
void BlockCalc(string infile, string outfile, const CoordTrans& trans, HistNodeND *blocktree)
{
	...
	else
	{
		...
		while (1)
		{
			...
			auto pt = inPt.Transform<double>(trans);
			convertpt[0] = round(pt[0]);
			convertpt[1] = round(pt[1]);
			convertpt[2] = round(pt[2]);
			convertpt[3] = round(pt[3]);
			...
			{
				output_file << fixed << setprecision(2) << inPt[i] << ", ";
			}
