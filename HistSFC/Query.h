...
private:
	...
	map <U, U> PlainWindowRange(const NDWindow<double> & window, short maxdepth, unsigned short dimbits = 20)
	{
		measure.histLoad = 0;
		map <U, U> ranges;
	...
	map <U, U> HistWindowRange2(const NDWindow<double>& window, short dimbits = 20)
	{
		//Directly search the histogram tree
		auto start = chrono::high_resolution_clock::now();
		map <U, U> ranges;
...
public:
	...
	virtual void QueryIOT(const NDWindow<T> & window)
	{
		...
		{
			auto start = chrono::high_resolution_clock::now();
			ranges = PlainWindowRange(windowQ, 4, dimbits);	//search depth can be modified depending on accuracy requirement
			auto end = chrono::high_resolution_clock::now();
			measure.rangeComp = chrono::duration_cast<chrono::milliseconds>(end - start).count();
			/*measure.histLoad = 0;
			auto start = chrono::high_resolution_clock::now();
			ranges = HistWindowRange2(windowQ, dimbits);
			auto end = chrono::high_resolution_clock::now();
			measure.rangeComp = chrono::duration_cast<chrono::milliseconds>(end - start).count();*/
		}
		...
		while (rs->next())
		{
			...
			if (Inside<int, double>(pt, windowQ) == 1)
			{
				...
				//output_file << ptreal[0] << ", " << ptreal[1] << ", " << ptreal[2] << "\n";
			}
