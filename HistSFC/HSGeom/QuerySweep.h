		...
		case ALG::VERTEX:
		{
			...
			for (auto it = vec.begin(); it != vec.end();)
			{
				...
				for (int i = 0; i < 1 << node.nDims; i++)
				{
					...
					if (num_intersects > 0 and num_intersects < i + 1)
					{
						res = 1;
						++it;
						break;
...
public:
	...
	void QueryView(const NDGeom & poly, const NDGeom & geom, ALG al)
	{
		alg = al;
		cycle_counter = 0;
		QueryGeom = geom;
		int dimnum = PCDB.nDims;
		NDGeom polytrans = poly.Transform(PCDB.trans).Normalize();
		...
		if (PCDB.HIST)
		{
			...
			auto start = chrono::high_resolution_clock::now();
			ranges = HistGeomRange(polytrans, al, dimbits);
			auto end = chrono::high_resolution_clock::now();
		...
		{
			auto start = chrono::high_resolution_clock::now();
			ranges = PlainGeomRange(polytrans, al, 12, dimbits);	//search depth can be modified depending on accuracy requirement
			auto end = chrono::high_resolution_clock::now();
			measure.rangeComp = chrono::duration_cast<chrono::milliseconds>(end - start).count();
		}
		...
		ofstream output_file("E:/querysweep_view_apx.csv");
		SFCConversion<int> sfc(dimnum, dimbits);
	...
	void QueryIOT(const NDGeom & geom, ALG al)
	{
		...
		{
			auto start = chrono::high_resolution_clock::now();
			ranges = PlainGeomRange(geomtrans2, al, 12, dimbits);	//search depth can be modified depending on accuracy requirement
			auto end = chrono::high_resolution_clock::now();
			measure.rangeComp = chrono::duration_cast<chrono::milliseconds>(end - start).count();
		}
